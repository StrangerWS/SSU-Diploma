...
public class CNN implements Serializable {
    ...
    private Runner getRunner() {
        if (runner == null)
            throw new NullPointerException("'runner' is null.  Set runner before start training or test!");
        return runner;
    }
    ...
    private char textify(int label) {
        switch (label) {
            case 1:
                return 'a';
            case 2:
    ...
    private void setConvErrors(final Layer layer, final Layer nextLayer) {
        ...
        final Process process = (start, end) -> {
